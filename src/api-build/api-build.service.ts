...
export class ApiBuildService {
  ...
  async executeApi(
    ...
    for (const api of apis) {
      ...
      const strippedPath = path.replace("/v1/api-build", "");

      const result = matcher(strippedPath);
      console.log({ path: api.path, strippedPath,  result });
      if (result) {
        matchedApi = api;
    ...
    switch (matchedApi.action) {
      case 'find':
        ...
        const findOneParams = matchedApi.params.filter(
          (p) => p.action === 'findOne',
        );
        const findAllParams = matchedApi.params.find(
          (p) => p.action === 'findAll',
        );
        const aggregateParams = matchedApi.params.find(
          (p) => p.action === 'aggregate',
        );
        ...
        if (findOneParams.length > 0) {
          if (Object.keys(filter).length === 0) {
            ...
            throw new BadRequestException(
              'No valid findOne parameters provided',
            );
          }
